...
template <> InputParameters validParams<ReturnMapIterDT>() {
  ...
  params.addRequiredParam<Real>(
      "max_iter", "Maximum of return map iteration that are admissible.");
  params.addRequiredParam<Real>(
      "min_iter", "Minimum of return map iteration that are admissible.");
  params.addRequiredParam<Real>(
      "ratio", "Ratio by which current time step is multiplied.");
...
ReturnMapIterDT::ReturnMapIterDT(const InputParameters &parameters)
    : TimeStepper(parameters), PostprocessorInterface(parameters),
      ...
      _min_iter(getParam<Real>("min_iter")), _ratio(getParam<Real>("ratio")),
      _dt_min(getParam<Real>("dt_min")), _dt_max(getParam<Real>("dt_max")) {}
