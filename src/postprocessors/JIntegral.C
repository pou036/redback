...
validParams<JIntegral>()
{
  ...
  params.addParam<unsigned int>("crack_front_point_index",
...
JIntegral::computeQpIntegral()
{
  ...
  grad_of_tensor_q(0, 1) = crack_direction(0) * _grad_of_scalar_q[ _qp ](1);
  grad_of_tensor_q(0, 2) = crack_direction(0) * _grad_of_scalar_q[ _qp ](2);
  grad_of_tensor_q(1, 0) = crack_direction(1) * _grad_of_scalar_q[ _qp ](0);
  grad_of_tensor_q(1, 1) = crack_direction(1) * _grad_of_scalar_q[ _qp ](1);
  grad_of_tensor_q(1, 2) = crack_direction(1) * _grad_of_scalar_q[ _qp ](2);
  grad_of_tensor_q(2, 0) = crack_direction(2) * _grad_of_scalar_q[ _qp ](0);
  grad_of_tensor_q(2, 1) = crack_direction(2) * _grad_of_scalar_q[ _qp ](1);
  grad_of_tensor_q(2, 2) = crack_direction(2) * _grad_of_scalar_q[ _qp ](2);

  Real eq = _Eshelby_tensor[ _qp ].doubleContraction(grad_of_tensor_q);
...
JIntegral::getValue()
{
  ...
  if (_convert_J_to_K)
    _integral_value =
      sign * std::sqrt(std::abs(_integral_value) * _youngs_modulus / (1 - std::pow(_poissons_ratio, 2)));
