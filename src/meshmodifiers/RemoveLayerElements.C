...
validParams<RemoveLayerElements>()
{
  ...
  params.addRequiredParam<SubdomainName>(
      "paired_block", "The paired block for which we add the boundary elements");
...
RemoveLayerElements::modify()
{
  ...
  Real porosity_change_threshold = 0.02;

  //do not change mesh for less than x porosity change
  if (std::abs(porosity_change) < porosity_change_threshold) return;
  ...
  if (porosity_change > 0)
    {
      _master_id = _mesh_ptr->getSubdomainID(getParam<SubdomainName>("master_block"));
      _paired_id = _mesh_ptr->getSubdomainID(getParam<SubdomainName>("paired_block"));
    }
  else
    {
      _paired_id = _mesh_ptr->getSubdomainID(getParam<SubdomainName>("master_block"));
      _master_id = _mesh_ptr->getSubdomainID(getParam<SubdomainName>("paired_block"));
    }
  ...
  for (const auto & elem : mesh.active_element_ptr_range())
  {
    ...
    N_elem += 1;
    if (elem->subdomain_id()==_paired_id) frac_volume+=elem->volume();
  }
  std::cout<<"total_volume = "<<total_volume<<std::endl;
  std::cout<<"frac_volume = "<<frac_volume<<std::endl;
  ...
  while (nb_loops < std::sqrt(N_elem))
  {
    ...
    while (nb_loops2 < 4)
    {
      ...
      if (layer_volume < volume_change_threshold) return;
    ...
    //dof_id_type e = 0; e < elements.size(); ++e)
    {
      ...
      for (auto & it : type2blocks)
      {
        ...
        {
          has_type = true;
          it.second.insert(_paired_id);
        }
        else if (it.second.count(_paired_id) > 0)
      ...
      if (!has_type)
      {
        ...
        blocks.insert(_paired_id);
        type2blocks.insert(std::make_pair(type, blocks));
      }

      elem->subdomain_id() = _paired_id;
    }
...
std::vector<Elem *>
RemoveLayerElements::BoundaryElements()
{
  std::vector<Elem *> elements;
  ...
  for (auto & elem : mesh.active_element_ptr_range())
  {
    ...
    if (elem->subdomain_id() != _master_id)
      continue;
    ...
    for (unsigned int side = 0; side < elem->n_sides(); side++)
    {
      ...
      else if (neighbor != NULL && neighbor->subdomain_id() == _paired_id)
      {
        Elem * elem1 = elem;
  ...
  if (!mesh.is_serial())
  {
    ...
    for (processor_id_type p = 1; p != my_n_proc; ++p)
    {
      ...
      for (const auto & q : query)
      {
        ...
        if (neighbor != NULL && neighbor->subdomain_id() == _paired_id)
        {
