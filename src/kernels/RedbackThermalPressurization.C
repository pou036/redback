...
template <> InputParameters validParams<RedbackThermalPressurization>() {
  ...
  params.addCoupledVar(
      "temperature", 0.0,
      "Temperature variable."); // TODO: check "required" fields across redback
...
RedbackThermalPressurization::RedbackThermalPressurization(
    ...
    : Kernel(parameters), _temp_dot(coupledDot("temperature")),
      _dtemp_dot_dtemp(coupledDotDu("temperature")),
      _pressurization_coefficient(
          getMaterialProperty<Real>("pressurization_coefficient")),
...
RedbackThermalPressurization::computeQpResidual() {
  return -_time_factor * _pressurization_coefficient[_qp] * _temp_dot[_qp] *
         _test[_i][_qp];
}

Real RedbackThermalPressurization::computeQpJacobian() { return 0; }

Real
RedbackThermalPressurization::computeQpOffDiagJacobian(unsigned int jvar) {
  if (jvar == _temp_var) {
    return -_time_factor * _pressurization_coefficient[_qp] *
           _dtemp_dot_dtemp[_qp] * _phi[_j][_qp] * _test[_i][_qp];
  }
  return 0;
}
