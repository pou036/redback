...
template <> InputParameters validParams<RedbackMassConvection>() {
...
RedbackMassConvection::RedbackMassConvection(const InputParameters &parameters)
    : Kernel(parameters), _grad_temp(coupledGradient("temperature")),
      //_dtemp_dot_dtemp(coupledDotDu("temperature")),

      _pressure_convective_mass(
          getMaterialProperty<RealVectorValue>("pressure_convective_mass")),
      _thermal_convective_mass(
          getMaterialProperty<RealVectorValue>("thermal_convective_mass")),
  ...
  return _time_factor * _test[_i][_qp] *
         (_pressure_convective_mass[_qp] * _grad_u[_qp] -
          _thermal_convective_mass[_qp] *
              _grad_temp[_qp]); // scalar product done by "*"
}
