...
template <> InputParameters validParams<RedbackMechDissip>() {
...
RedbackMechDissip::RedbackMechDissip(const InputParameters &parameters)
    : Kernel(parameters),
      _mechanical_dissipation(
          hasMaterialProperty<Real>("mechanical_dissipation_mech")
      ...
      _mechanical_dissipation_jac(
          hasMaterialProperty<Real>("mechanical_dissipation_jacobian_mech")
              ? getMaterialProperty<Real>(
                    "mechanical_dissipation_jacobian_mech")
              : getMaterialProperty<Real>(
                    "mechanical_dissipation_jacobian_no_mech")),
...
RedbackMechDissip::computeQpJacobian() {
  return -_time_factor * _test[_i][_qp] * _mechanical_dissipation_jac[_qp] *
         _phi[_j][_qp];
}
