...
template <>
InputParameters
validParams<RedbackStressDivergenceTensors_UO>()
{
...
RedbackStressDivergenceTensors_UO::RedbackStressDivergenceTensors_UO(const InputParameters & parameters) :
    Kernel(parameters),
    ...
    _gravity_term(getMaterialProperty<RealVectorValue>("mixture_gravity_term"))
{
	...
	// nb redback params are currently only needed for coupled pore pressure.
...
Real
RedbackStressDivergenceTensors_UO::computeQpResidual()
{
  ...
  if (_pore_pres_coupled)
  {
    ...
    _poromech_stress_row(_component) -= (*_biot_coeff_uo)[ _qp ] * _pore_pres[ _qp ];
...
Real
RedbackStressDivergenceTensors_UO::computeQpJacobian()
{
...
Real
RedbackStressDivergenceTensors_UO::computeQpOffDiagJacobian(unsigned int jvar)
{
