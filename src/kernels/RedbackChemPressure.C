...
RedbackChemPressure::RedbackChemPressure(const InputParameters &parameters)
    : Kernel(parameters),
      ...
      _chemical_source_mass_jac(
          getMaterialProperty<Real>("chemical_source_mass_jacobian")),
...
Real RedbackChemPressure::computeQpJacobian() { return 0; }

Real
RedbackChemPressure::computeQpOffDiagJacobian(unsigned int jvar) {
  if (jvar == _temp_var) {
    return -_time_factor * _test[_i][_qp] * _chemical_source_mass_jac[_qp] *
           _phi[_j][_qp];
  }
  return 0;
}
