...
template <> InputParameters validParams<RedbackMaterial>() {
  InputParameters params = validParams<Material>();

  params.addParam<std::vector<std::string> >(
      "init_from_functions__params",
  ...
  params.addRangeCheckedParam<Real>("phi0", 0.0, "phi0>=0 & phi0<1",
  ...
  params.addParam<Real>("alpha_1", 0.0, "First parameter for activation "
                                        "volume, alpha_1 V_{ref} / (R T_{ref}) "
                                        "in the redback paper");
  params.addParam<Real>("alpha_2", 0.0, "Second parameter for activation "
                                        "volume, alpha_2 V_{ref} / (R T_{ref}) "
                                        "in the redback paper");
  params.addParam<Real>(
      "alpha_3", 0.0,
      "Third parameter for activation volume, alpha_3 in the redback paper");
  params.addParam<Real>("confining_pressure", 1,
  ...
  params.addParam<bool>("are_convective_terms_on", false,
                        "are convective terms on?");
  ...
  params.addCoupledVar("total_porosity", 0.0,
                       "The total porosity (as AuxKernel)");
  params.addCoupledVar("inverse_lewis_number_tilde", 0.0,
  ...
  params.addParam<MooseEnum>(
      "density_method", RedbackMaterial::densityMethodEnum() = "linear",
  ...
  params.addParam<Real>("ar_F", 0.0,
                        "Arrhenius number for the forward reaction.");
  params.addParam<Real>("ar_R", 0.0,
                        "Arrhenius number for the reverse reaction.");
  params.addParam<Real>("da_endo", 0,
                        "Damkoehler number for the endothermic reaction.");
  params.addParam<Real>("da_exo", 0,
                        "Damkoehler number for the exothermic reaction.");
  params.addParam<Real>("mu", 0, "Chemical pressurization coefficient.");
  params.addParam<Real>("Kc", 0, "Equilibrium constant.");
  params.addParam<Real>("eta1", 1,
                        "ratio of concentrations (see documentation).");
  params.addParam<Real>("eta2", 1,
                        "ratio of concentrations (see documentation).");
  params.addRangeCheckedParam<Real>("Aphi", 0.0, "Aphi>=0 & Aphi<=1",
                                    "percentage of volume change from "
                                    "chemistry contributing to porosity (see "
                                    "documentation)");
  params.addParam<Real>("pressurization_coefficient", 0,
                        "Pressurization coefficient (Lambda).");
  params.addParam<Real>("Peclet_number", 1, "Peclet number");

  params.addParam<Real>("solid_compressibility", 1,
                        "solid compressibility (beta^{(s)} in "
                        "1/Pa)"); // _solid_compressibility_param
  params.addParam<Real>("fluid_compressibility", 0,
  ...
  params.addParam<Real>(
      "fluid_thermal_expansion", 0,
      "fluid expansion (lambda^{(f)} in 1/K)"); // _fluid_thermal_expansion_param
  params.addParam<Real>("solid_density", 2.5,
                        "normalised solid density (-)"); // solid_density_param
  params.addParam<Real>("fluid_density", 1,
                        "normalised fluid density (-)"); // fluid_density_param

  params.addParam<RealVectorValue>("gravity", RealVectorValue(),
                                   "Gravitational acceleration (m/s^2) as a "
                                   "vector pointing downwards.  Eg "
                                   "(0,0,-9.81)");

  params.addParam<Real>("temperature_reference", 0.0,
                        "Reference temperature used for thermal expansion");
  params.addParam<Real>("pressure_reference", 0.0,
...
RedbackMaterial::RedbackMaterial(const InputParameters &parameters)
    : Material(parameters), _has_T(isCoupled("temperature")),
      ...
      _init_from_functions__params(
          getParam<std::vector<std::string> >("init_from_functions__params")),
      _init_from_functions__function_names(getParam<std::vector<FunctionName> >(
          "init_from_functions__function_names")),
      _phi0_param(getParam<Real>("phi0")), _gr_param(getParam<Real>("gr")),
      _ref_lewis_nb_param(getParam<Real>("ref_lewis_nb")),
      _ar_param(getParam<Real>("ar")), _delta_param(getParam<Real>("delta")),
      ...
      _ar_F_param(getParam<Real>("ar_F")), _ar_R_param(getParam<Real>("ar_R")),
      _da_endo_param(getParam<Real>("da_endo")),
      _da_exo_param(getParam<Real>("da_exo")), _mu_param(getParam<Real>("mu")),
      _Kc_param(getParam<Real>("Kc")), _eta1_param(getParam<Real>("eta1")),
      _eta2_param(getParam<Real>("eta2")), _Aphi_param(getParam<Real>("Aphi")),
      _pressurization_coefficient_param(
          getParam<Real>("pressurization_coefficient")),
      ...
      _mixture_gravity_term(declareProperty<RealVectorValue>(
          "mixture_gravity_term")), // rho_mixture * g
      _fluid_gravity_term(declareProperty<RealVectorValue>(
          "fluid_gravity_term")), // rho_fluid * g
      ...
      _mixture_compressibility(
          declareProperty<Real>("mixture_compressibility")),

      _mechanical_dissipation_no_mech(
          declareProperty<Real>("mechanical_dissipation_no_mech")),
      ...
      _ar_R(declareProperty<Real>("ar_R")), _mu(declareProperty<Real>("mu")),
      ...
      _chemical_endothermic_energy(
          declareProperty<Real>("chemical_endothermic_energy")),
      _chemical_endothermic_energy_jac(
          declareProperty<Real>("chemical_endothermic_energy_jacobian")),
      _chemical_exothermic_energy(
          declareProperty<Real>("chemical_exothermic_energy")),
      _chemical_exothermic_energy_jac(
          declareProperty<Real>("chemical_exothermic_energy_jacobian")),
      _chemical_source_mass(declareProperty<Real>("chemical_source_mass")),
      _chemical_source_mass_jac(
          declareProperty<Real>("chemical_source_mass_jacobian")),

      _thermal_convective_mass(
          declareProperty<RealVectorValue>("thermal_convective_mass")),
      _pressure_convective_mass(
          declareProperty<RealVectorValue>("pressure_convective_mass")),
      ...
      _mixture_convective_energy(
          declareProperty<RealVectorValue>("mixture_convective_energy")),
      ...
      _solid_thermal_expansion(
          declareProperty<Real>("solid_thermal_expansion")),
      _fluid_thermal_expansion(
          declareProperty<Real>("fluid_thermal_expansion")),

      _mixture_density(declareProperty<Real>("mixture_density")),

      _density_method((DensityMethod)(int)
                      getParam<MooseEnum>("density_method")),
      _permeability_method((PermeabilityMethod)(int)
                           getParam<MooseEnum>("permeability_method")),

      _mises_strain(declareProperty<Real>("mises_strain")),
      _mises_strain_rate(declareProperty<Real>("mises_strain_rate")),

      _pressurization_coefficient(
          declareProperty<Real>("pressurization_coefficient")),
...

{
  ...
  for (unsigned int i = 0; i < _num_init_functions; i++) {
    pos = find(valid_params.begin(), valid_params.end(),
    ...
    _init_functions[i] =
        &getFunctionByName(_init_from_functions__function_names[i]);
  }
}

MooseEnum RedbackMaterial::densityMethodEnum() { return MooseEnum("linear"); }
...
RedbackMaterial::stepInitQpProperties() {
  ...
  _mises_strain[_qp] = 0;
  _solid_velocity[_qp] =
      RealVectorValue(_dispx_dot[_qp], _dispy_dot[_qp], _dispz_dot[_qp]);
  ...
  pos = find(_init_from_functions__params.begin(),
             _init_from_functions__params.end(), "gr") -
  ...
  pos = find(_init_from_functions__params.begin(),
             _init_from_functions__params.end(), "ref_lewis_nb") -
        _init_from_functions__params.begin();
  if (pos < _num_init_functions) {
  ...
  pos = find(_init_from_functions__params.begin(),
             _init_from_functions__params.end(), "ar") -
        _init_from_functions__params.begin();
  if (pos < _num_init_functions) {
  ...
  _mixture_density[_qp] = (1 - _phi0_param) * _solid_density_param +
                          _phi0_param * _fluid_density_param;
...
RedbackMaterial::computeRedbackTerms() {
  ...
  if (!_is_mechanics_on) {
    ...
    _mechanical_dissipation_no_mech[_qp] =
        ...
        std::exp(_ar[_qp] * _delta[_qp] * _T[_qp] /
                 (1 + _delta[_qp] * _T[_qp]));

    // Compute Mechanical Dissipation Jacobian
    _mechanical_dissipation_jac_no_mech[_qp] =
        _mechanical_dissipation_no_mech[_qp] * _ar[_qp] * _delta[_qp] /
  ...
  if (_is_chemistry_on) {
    ...
    */
    ...
    omega_rel = _eta2_param * _Kc_param * std::exp(-(_ar_F[_qp] - _ar_R[_qp]) /
                                                   (1 + _delta[_qp] * _T[_qp]));
    ...
    _chemical_porosity[_qp] = _Aphi_param * (1 - _phi0_param) *
                              _solid_ratio[_qp] /
                              (_solid_ratio[_qp] + _eta1_param);
    ...
    temporary =
        _eta2_param * _Kc_param * (_ar_F[_qp] - _ar_R[_qp]) * _delta[_qp] *
    ...
    s_prime = -temporary /
              std::pow(std::exp(_ar_R[_qp] / (1 + _delta[_qp] * _T[_qp])) +
                           std::exp(_ar_F[_qp] / (1 + _delta[_qp] * _T[_qp])) *
                               _eta2_param * _Kc_param,
                       2);
    ...
    _chemical_endothermic_energy[_qp] =
        ...
        std::exp((_ar_F[_qp] * _delta[_qp] * _T[_qp]) /
                 (1 + _delta[_qp] * _T[_qp]));
    ...
    _chemical_endothermic_energy_jac[_qp] =
        ...
        (_ar_F[_qp] * _delta[_qp] * (1 - _total_porosity[_qp]) *
             (1 - _solid_ratio[_qp]) / std::pow(1 + _delta[_qp] * _T[_qp], 2) -
    ...
    _chemical_exothermic_energy[_qp] =
        _da_exo_param * (1 - _total_porosity[_qp]) * _solid_ratio[_qp] *
        _chemical_porosity[_qp] *
        std::exp((_ar_R[_qp] * _delta[_qp] * _T[_qp]) /
                 (1 + _delta[_qp] * _T[_qp]));
    ...
    _chemical_exothermic_energy_jac[_qp] =
        ...
        (_solid_ratio[_qp] *
             (_ar_R[_qp] * _delta[_qp] * _chemical_porosity[_qp] *
                  (1 - _total_porosity[_qp]) /
                  std::pow(1 + _delta[_qp] * _T[_qp], 2) +
              (1 - _total_porosity[_qp] - _chemical_porosity[_qp]) *
                  phi_prime) +
    ...
    _chemical_source_mass[_qp] = _mu[_qp] * (1 - _total_porosity[_qp]) *
                                 (1 - _solid_ratio[_qp]) *
                                 std::exp((_ar_F[_qp] * _delta[_qp] * _T[_qp]) /
                                          (1 + _delta[_qp] * _T[_qp]));
...
- (1 - _total_porosity[_qp]) * s_prime
    ...
    Real inverse_lewis_number =
        1 / _lewis_number[_qp] +
        _inverse_lewis_number_tilde[_qp]; // to include modification from
  ...
  phi_beta_star_f =
      _total_porosity[_qp] *
      _fluid_compressibility[_qp]; // normalized compressibility of the fluid
                                   // phase
  beta_star_m = one_minus_phi_beta_star_s +
                phi_beta_star_f; // normalized compressibility of the mixture
  ...
  if (_are_convective_terms_on) {
    ...
    _mixture_density[_qp] = (1 - _total_porosity[_qp]) * solid_density +
                            _total_porosity[_qp] * fluid_density;
    ...
    _mixture_gravity_term[_qp] =
        ...
        normalized_gravity; // for the stress equilibrium equation
    _fluid_gravity_term[_qp] =
        fluid_density * normalized_gravity; // for Darcy's flux
    ...
    phi_lambda_f = _total_porosity[_qp] *
                   _fluid_thermal_expansion[_qp]; // normalized thermal
                                                  // expansion coefficient of
                                                  // the fluid phase
    lambda_m_star = one_minus_phi_lambda_s +
                    phi_lambda_f; // normalized compressibility of the mixture

    // Forming the velocities through mechanics and Darcy's flow law
    _fluid_velocity[_qp] = _solid_velocity[_qp] -
                           beta_star_m * (_grad_pore_pressure[_qp] -
                                          fluid_density * normalized_gravity) /
    ...
    _pressure_convective_mass[_qp] =
        ...
        ((one_minus_phi_beta_star_s / beta_star_m) * _solid_velocity[_qp] +
         (phi_beta_star_f / beta_star_m) *
             _fluid_velocity[_qp]); // convective term multiplying
