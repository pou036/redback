...
validParams<RedbackMechMaterialLne>()
{
  ...
  params.addParam<Real>("alpha", 1, "material parameter controlling location (in p') of yield surface maximum");
 ...
 */
Real
RedbackMechMaterialLne::getFlowIncrement(
                              ...
                              RankFourTensor & dresid_dsig)
{
  ...
  for (i = 0; i < 3; ++i)
    for (j = 0; j < 3; ++j)
      for (k = 0; k < 3; ++k)
        for (l = 0; l < 3; ++l)
          dfi_dsig(i, j, k, l) = flow_dirn(i, j) * (f0 * sig_dev(k, l) * dfi_dseqv + dfi_dp * deltaFunc(k, l) / 3.0);
  ...
  for (i = 0; i < 3; ++i)
    for (j = 0; j < 3; ++j)
      for (k = 0; k < 3; ++k)
        for (l = 0; l < 3; ++l)
          dfd_dsig(i, j, k, l) =
            f1 * deltaFunc(i, j) * deltaFunc(k, l) + f2 * deltaFunc(i, k) * deltaFunc(j, l) -
            (f3 * deltaFunc(i, j) + f4 * sig_dev(i, j)) * (f5 * deltaFunc(k, l) + f6 * sig_dev(k, l));
