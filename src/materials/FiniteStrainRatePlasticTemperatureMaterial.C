...
/**
FiniteStrainRatePlasticTemperatureMaterial integrates the rate dependent J2 plasticity model (Overstress model) in a
...
void
FiniteStrainRatePlasticTemperatureMaterial::initQpStatefulProperties()
{
...
void
FiniteStrainRatePlasticTemperatureMaterial::computeQpStress()
{
 ...
 */
void
FiniteStrainRatePlasticTemperatureMaterial::returnMap(const RankTwoTensor & sig_old, const RankTwoTensor & delta_d, const RankFourTensor & E_ijkl, RankTwoTensor & dp, RankTwoTensor & sig)
{
  ...
  _mises_stress[_qp] =  getSigEqv(sig_new);
  _mech_dissipation[_qp] =  _mises_stress[_qp] * _ref_pe_rate * std::pow(macaulayBracket(getSigEqv(sig_new) / yield_stress - 1.0), _exponent)*_exponential;
...
void
FiniteStrainRatePlasticTemperatureMaterial::getFlowTensor(const RankTwoTensor & sig, Real /*yield_stress*/, RankTwoTensor & flow_tensor)
{
...
void
FiniteStrainRatePlasticTemperatureMaterial::getJac(const RankTwoTensor & sig, const RankFourTensor & E_ijkl, Real flow_incr, Real yield_stress, RankFourTensor & dresid_dsig)
{
  unsigned i, j, k ,l;
...
Real
FiniteStrainRatePlasticTemperatureMaterial::macaulayBracket(Real val)
{
  if (val > 0.0)
    return val;
  else
