...
template <>
InputParameters
validParams<RedbackMechMaterialCC_UO_DC>()
{
  InputParameters params = validParams<RedbackMechMaterial_UO_DC>();
...
RedbackMechMaterialCC_UO_DC::RedbackMechMaterialCC_UO_DC(const InputParameters & parameters) :
    RedbackMechMaterial_UO_DC(parameters), _slope_yield_surface(getParam<Real>("slope_yield_surface"))
{
}
 ...
 */
void
RedbackMechMaterialCC_UO_DC::getFlowTensor(const RankTwoTensor & sig, Real q, Real p, Real pc, RankTwoTensor & flow_tensor)
{
  if (pc > 0)
    pc *= -1;
 ...
 */
Real
RedbackMechMaterialCC_UO_DC::getFlowIncrement(Real sig_eqv, Real pressure, Real q_yield_stress, Real p_yield_stress, Real pc)
{
  pc *= -1;
...
Real
RedbackMechMaterialCC_UO_DC::getDerivativeFlowIncrement(
...
void
RedbackMechMaterialCC_UO_DC::getJac(const RankTwoTensor & sig,
...
void
RedbackMechMaterialCC_UO_DC::get_py_qy(Real p, Real q, Real & p_y, Real & q_y, Real yield_stress)
{
  Ellipse::distanceCC(_slope_yield_surface, -yield_stress, p, q, p_y, q_y);
}

void
RedbackMechMaterialCC_UO_DC::form_damage_kernels(Real q_y)
{
}
