...
template <> InputParameters validParams<RedbackMechMaterial>() {
  ...
  params.addParam<Real>("eptol", 1e-7,
  ...
  params.addParam<Real>("ref_pe_rate", 1.0, "Reference plastic strain rate "
                                            "parameter for rate dependent "
                                            "plasticity (Overstress model)");
  params.addParam<Real>("exponent", 1.0,
  ...
  params.addParam<MooseEnum>(
      "damage_method", RedbackMechMaterial::damageMethodEnum() = "CreepDamage",
...
RedbackMechMaterial::RedbackMechMaterial(const InputParameters &parameters)
    : Material(parameters),
      ...
      _total_volumetric_strain(
          declareProperty<Real>("total_volumetric_strain")),
      ...
      _damage(coupledValue("damage")), _damage_old(coupledValueOld("damage")),
      ...
      _solid_compressibility(
          getMaterialProperty<Real>("solid_compressibility")),
...
RedbackMechMaterial::computeStrain() {
  ...
  for (_qp = 0; _qp < _qrule->n_points(); ++_qp) {
    ...
    RankTwoTensor A(_grad_disp_x[_qp], _grad_disp_y[_qp],
                    _grad_disp_z[_qp]); // Deformation gradient
    RankTwoTensor Fbar(_grad_disp_x_old[_qp], _grad_disp_y_old[_qp],
...
RedbackMechMaterial::computeQpStress() {
  ...
  returnMap(_stress_old[_qp], _strain_increment[_qp], _elasticity_tensor[_qp],
            dp, sig, p_y, q_y);
  _stress[_qp] = sig;
  ...
  RankTwoTensor delta_ee = _strain_increment[_qp] -
...
// Delta Function
Real
RedbackMechMaterial::deltaFunc(const unsigned int i,
      ...
      if (eqpe > _yield_stress_vector[ind] &&
        ...
        return _yield_stress_vector[ind + 1] +
               (eqpe - _yield_stress_vector[ind]) /
                   ...
                   (_yield_stress_vector[ind + 3] -
                    _yield_stress_vector[ind + 1]);
    } else
...
void
RedbackMechMaterial::computeRedbackTerms(RankTwoTensor &sig, Real q_y,
                                              Real p_y) {
...
RedbackMechMaterial::returnMap(const RankTwoTensor &sig_old,
                                    ...
                                    RankTwoTensor &dp, RankTwoTensor &sig,
                                    Real &p_y, Real &q_y) {
  ...
  if (_has_T) {
    // E_0/(RT) = Ar/(1+delta T*)
    _exponential =
        std::exp(-_ar[_qp]) * std::exp(_ar[_qp] * _delta[_qp] * _T[_qp] /
                                       (1 + _delta[_qp] * _T[_qp]));
  }
...
void
RedbackMechMaterial::get_py_qy_damaged(Real p, Real q, Real &p_y,
...
RedbackMechMaterial::formDamageDissipation(RankTwoTensor &sig) {
  ...
  shear_modulus = 0.5 * _youngs_modulus /
                  (1 + _poisson_ratio); // Second Lame modulus (shear)
