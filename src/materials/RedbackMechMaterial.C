...
RedbackMechMaterial::RedbackMechMaterial(const InputParameters & parameters)
  : Material(parameters),
    ...
    _elastic_volumetric_strain(declareProperty<Real>("elastic_volumetric_strain")),
    ...
    _alpha_4(getMaterialProperty<Real>("alpha_4")),
...
RedbackMechMaterial::initQpStatefulProperties()
{
  ...
  _elastic_volumetric_strain[ _qp ] = 0;
...
RedbackMechMaterial::computeQpStress()
{
  ...
  _elastic_volumetric_strain[ _qp ] = _elastic_strain[ _qp ].trace();
                               ...
                               Real & q_y)
{
  ...
  while (err3 > tol3 && iterisohard < maxiterisohard) // Hardness update iteration
  {
    ...
    if (is_first_plastic)
    {
      ...
      {
        throw MooseException("Constitutive Error-Too many iterations (" +
                             Moose::stringify(maxiter) + ", err1=" + Moose::stringify(err1) +
                             " > tol1=" + Moose::stringify(tol1) +
                             "): Reduce time "
                             "increment.\n");
      }
    }
