...
template <>
InputParameters
validParams<RedbackFluidMaterial_UO>()
{
...
RedbackFluidMaterial_UO::RedbackFluidMaterial_UO(const InputParameters & parameters) :
    Material(parameters),
    ...
    _P0_param(getParam<Real>("pressure_reference"))
{
	 ...
	 // common redback material parameters
	  ...
	  _froude_number_uo = _common_redback_material_parameters->GetRequiredParameterObject(RedbackParameters::FroudeNumber.str);

	  _fluid_compressibility_uo = _common_redback_material_parameters->GetRequiredParameterObject(RedbackParameters::fluidCompressiblity.str);
	  _fluid_thermal_expansion_uo = _common_redback_material_parameters->GetRequiredParameterObject(RedbackParameters::fluidThermalExpansion.str);
...
void
RedbackFluidMaterial_UO::stepInitQpProperties()
{
  _fluid_density[ _qp ] = _fluid_density_param;
}

void
RedbackFluidMaterial_UO::computeQpProperties()
{
  // Set our variables
...
void
RedbackFluidMaterial_UO::computeRedbackTerms()
{
  /*
  ...
  */
  ...
  Real fluid_density;
  fluid_density = (1 + (*_fluid_compressibility_uo)[ _qp ] * _pore_pres[ _qp ] - (*_fluid_thermal_expansion_uo)[ _qp ] * _T[ _qp ]);

  // Gravity term in the momentum kernel
  _gravity_term[ _qp ] = _gravity_param / pow( (*_froude_number_uo)[ _qp ], 2);
  ...
  _div_fluid_kernel[ _qp ] = _div_fluid_vel[ _qp ] * (*_peclet_number_uo)[ _qp ] / (*_fluid_compressibility_uo)[ _qp ];

  // Assembling mass kernels components
  _pressurization_coefficient[ _qp ] =  (*_fluid_thermal_expansion_uo)[ _qp ]/ (*_fluid_compressibility_uo)[ _qp ];
  ...
  _pressure_convective_mass[ _qp ] = fluid_vel_vector *  (*_peclet_number_uo)[ _qp ];
  _thermal_convective_mass[ _qp ] = _pressurization_coefficient[ _qp ] * fluid_vel_vector * (*_peclet_number_uo)[ _qp ];
