...
template <> InputParameters validParams<RedbackMechMaterialCC>() {
  ...
  params.addParam<Real>("slope_yield_surface", 0,
 ...
 */
void
RedbackMechMaterialCC::getFlowTensor(const RankTwoTensor &sig, Real q,
                                          Real p, Real pc,
  ...
  flow_tensor =
      3.0 * sig.deviatoric() / (_slope_yield_surface * _slope_yield_surface);
 ...
 */
Real
RedbackMechMaterialCC::getFlowIncrement(Real sig_eqv, Real pressure,
                                             Real q_yield_stress,
                                             Real p_yield_stress, Real pc) {
  pc *= -1;
  if (Ellipse::isPointOutsideOfEllipse(/*m=*/_slope_yield_surface, /*p_c=*/pc,
                                       /*x=*/pressure, /*y=*/sig_eqv)) {
    Real flow_incr_vol =
        _ref_pe_rate * _dt *
...
Real
RedbackMechMaterialCC::getDerivativeFlowIncrement(const RankTwoTensor &sig,
                                                       Real pressure,
                                                       Real sig_eqv, Real pc,
                                                       Real q_yield_stress,
                                                       Real p_yield_stress) {
  if (Ellipse::isPointOutsideOfEllipse(/*m=*/_slope_yield_surface, /*p_c=*/pc,
                                       /*x=*/pressure, /*y=*/sig_eqv)) {
...
RedbackMechMaterialCC::getJac(const RankTwoTensor &sig,
                                   const RankFourTensor &E_ijkl, Real flow_incr,
                                   Real sig_eqv, Real pressure,
                                   Real p_yield_stress, Real q_yield_stress,
...
void
RedbackMechMaterialCC::get_py_qy(Real p, Real q, Real &p_y, Real &q_y,
                                      Real yield_stress) {
