...
template <> InputParameters validParams<RedbackMechMaterialCCanisotropic>() {
  InputParameters params = validParams<RedbackMechMaterialCC>();
  params.addParam<Real>("initial_anisotropy_param", 0,
                        "Initial anisotropy coefficient (from Dafalias 2013)");

  return params;
}

RedbackMechMaterialCCanisotropic::RedbackMechMaterialCCanisotropic(
...
RedbackMechMaterialCCanisotropic::stepInitQpProperties() {
  ...
  if (_t == 0)
    _anisotropy_coeff[_qp] =
        _initial_anisotropy_param; // TODO: implement \dot{alpha}
  else
    _anisotropy_coeff[_qp] =
        _anisotropy_coeff[_qp] +
        0 * (_slope_yield_surface - _anisotropy_coeff[_qp]) *
 ...
 */
void
RedbackMechMaterialCCanisotropic::getFlowTensor(const RankTwoTensor &sig,
                                                Real q, Real p, Real pc,
  ...
  flow_tensor = 3.0 * ((q - _anisotropy_coeff[_qp] * p) / q) *
                sig.deviatoric() / (M_squared - alpha_squared);
  flow_tensor.addIa(2.0 * p - pc -
                    2.0 * _anisotropy_coeff[_qp] *
                        (q - _anisotropy_coeff[_qp] * p) /
                        (M_squared - alpha_squared) / 3.0); //(p > 0 ? 1:-1)
 ...
 */
Real
RedbackMechMaterialCCanisotropic::getFlowIncrement(Real sig_eqv,
                                                        Real pressure,
                                                        Real q_yield_stress,
                                                        Real p_yield_stress,
                                                        Real pc) {
  ...
  if (Ellipse::isPointOutsideOfRotatedEllipse(/*m=*/_slope_yield_surface,
    ...
    Real flow_incr_vol =
        _ref_pe_rate * _dt *
        std::pow(std::fabs(pressure - p_yield_stress), _exponent) *
        _exponential;
    Real flow_incr_dev =
        _ref_pe_rate * _dt *
        std::pow(std::fabs(sig_eqv - q_yield_stress), _exponent) * _exponential;
    return std::pow(
        flow_incr_vol * flow_incr_vol + flow_incr_dev * flow_incr_dev, 0.5);
  } else
    return 0;
}
...
RedbackMechMaterialCCanisotropic::getDerivativeFlowIncrement(
    const RankTwoTensor &sig, Real pressure, Real sig_eqv, Real pc,
    Real q_yield_stress, Real p_yield_stress) {
  if (Ellipse::isPointOutsideOfRotatedEllipse(/*m=*/_slope_yield_surface,
                                              /*p_0=*/pc,
                                              /*alpha=*/_anisotropy_coeff[_qp],
                                              /*x=*/pressure,
    ...
    Real delta_lambda_q =
        _ref_pe_rate * _dt *
        std::pow(std::fabs(sig_eqv - q_yield_stress), _exponent) * _exponential;
    Real delta_lambda = (std::pow(delta_lambda_p * delta_lambda_p +
                                      delta_lambda_q * delta_lambda_q,
                                  0.5));
    Real der_flow_incr_dev =
        _ref_pe_rate * _dt * _exponent *
        std::pow(std::fabs(sig_eqv - q_yield_stress), _exponent - 1.0) *
        _exponential;
    Real der_flow_incr_vol =
        _ref_pe_rate * _dt * _exponent *
        std::pow(std::fabs(pressure - p_yield_stress), _exponent - 1.0) *
        _exponential;
...
void
RedbackMechMaterialCCanisotropic::getJac(
    const RankTwoTensor &sig, const RankFourTensor &E_ijkl, Real flow_incr,
    Real sig_eqv, Real pressure, Real p_yield_stress, Real q_yield_stress,
    Real pc, RankFourTensor &dresid_dsig) {
  unsigned i, j, k, l;
  ...
  dfi_dseqv = getDerivativeFlowIncrement(sig, pressure, sig_eqv, pc,
                                         q_yield_stress, p_yield_stress);
  ...
  if (sig_eqv > 1e-8) {
    f1 = 3.0 / (_slope_yield_surface * _slope_yield_surface -
                _anisotropy_coeff[_qp] * _anisotropy_coeff[_qp]);
    ...
    f5 = f1 * (3.0 * _anisotropy_coeff[_qp] * p_yield_stress /
               (2.0 * q_yield_stress * q_yield_stress));
  }
  for (i = 0; i < 3; ++i)
    for (j = 0; j < 3; ++j)
      for (k = 0; k < 3; ++k)
        for (l = 0; l < 3; ++l)
          dfd_dsig(i, j, k, l) =
              ...
              f3 * deltaFunc(i, j) * deltaFunc(k, l) +
              f4 * sig_dev(i, j) * deltaFunc(k, l) +
              f5 * sig_dev(i, j) * sig_dev(k, l) +
              f4 * deltaFunc(i, j) *
                  sig_dev(k, l); // d_flow_dirn/d_sig - 2nd part
...
void
RedbackMechMaterialCCanisotropic::get_py_qy(Real p, Real q, Real &p_y,
                                                 Real &q_y, Real yield_stress) {
  Ellipse::distanceCCanisotropic(_slope_yield_surface, -yield_stress,
                                 _anisotropy_coeff[_qp], p, q, p_y, q_y);
}
