...
template <> InputParameters validParams<RedbackMechMaterialCCanisotropic>()
{
  ...
  params.addParam<Real>("initial_anisotropy_param",
                        0,
 ...
 */
void RedbackMechMaterialCCanisotropic::getFlowTensor(
 ...
 */
Real RedbackMechMaterialCCanisotropic::getFlowIncrement(
Real sig_eqv, Real pressure, Real q_yield_stress, Real p_yield_stress, Real pc)
{
                                              ...
                                              /*y=*/sig_eqv))
  {
    Real flow_incr_vol =
    _ref_pe_rate * _dt *
    std::pow(std::fabs(pressure - p_yield_stress), _exponent) * _exponential;
    Real flow_incr_dev =
    _ref_pe_rate * _dt *
...
Real RedbackMechMaterialCCanisotropic::getDerivativeFlowIncrement(
...
void RedbackMechMaterialCCanisotropic::getJac(const RankTwoTensor & sig,
                                              ...
                                              RankFourTensor & dresid_dsig)
{
  ...
  sig, pressure, sig_eqv, pc, q_yield_stress, p_yield_stress);
...
void RedbackMechMaterialCCanisotropic::get_py_qy(
Real p, Real q, Real & p_y, Real & q_y, Real yield_stress)
{
  Ellipse::distanceCCanisotropic(
  _slope_yield_surface, -yield_stress, _anisotropy_coeff[_qp], p, q, p_y, q_y);
}
