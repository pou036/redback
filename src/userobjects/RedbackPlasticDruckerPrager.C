...
#include "RedbackPlasticDruckerPrager.h"
#include "RankFourTensor.h"
#include "libmesh/utility.h"

registerMooseObject("RedbackApp", RedbackPlasticDruckerPrager);
    ...
    _zero_psi_hardening(_mc_psi.modelName().compare("Constant") == 0)
{
  if (_mc_phi.value(0.0) < 0.0 || _mc_psi.value(0.0) < 0.0 ||
    ...
    mooseError("RedbackPlasticDruckerPrager: MC friction and dilation angles must lie in "
               "[0, Pi/2]");
  if (_mc_phi.value(0) < _mc_psi.value(0.0))
    mooseError("RedbackPlasticDruckerPrager: MC friction angle must not be less than MC "
               "dilation angle");
  if (_mc_cohesion.value(0.0) < 0)
    mooseError("RedbackPlasticDruckerPrager: MC cohesion should not be negative");
...
Real
RedbackPlasticDruckerPrager::yieldFunction(const RankTwoTensor & stress, Real intnl) const
{
  Real aaa;
  Real bbb;
...
RankTwoTensor
RedbackPlasticDruckerPrager::dyieldFunction_dstress(const RankTwoTensor & stress,
                                                            Real intnl) const
{
  Real bbb;
...
Real
RedbackPlasticDruckerPrager::dyieldFunction_dintnl(const RankTwoTensor & stress,
                                                           Real intnl) const
{
  Real daaa;
  Real dbbb;
...
RankTwoTensor
RedbackPlasticDruckerPrager::flowPotential(const RankTwoTensor & stress, Real intnl) const
{
  Real bbb_flow;
...
RankFourTensor
RedbackPlasticDruckerPrager::dflowPotential_dstress(const RankTwoTensor & stress,
                                                            Real /*intnl*/) const
{
  RankFourTensor dr_dstress;
  dr_dstress = 0.5 * stress.d2secondInvariant() * std::sqrt(3.0 / stress.secondInvariant());
  ...
  dr_dstress += -0.5 * 0.5 * std::sqrt(3.0) * stress.dsecondInvariant().outerProduct(stress.dsecondInvariant()) /
                std::pow(stress.secondInvariant(), 1.5);
  return dr_dstress;
}

RankTwoTensor
RedbackPlasticDruckerPrager::dflowPotential_dintnl(const RankTwoTensor & stress,
                                                           Real intnl) const
{
  Real dbbb;
...
std::string
RedbackPlasticDruckerPrager::modelName() const
{
  return "DruckerPrager";
}

void
RedbackPlasticDruckerPrager::bothAB(Real intnl, Real & aaa, Real & bbb) const
{
...
void
RedbackPlasticDruckerPrager::onlyB(Real intnl, int fd, Real & bbb) const
{
...
void
RedbackPlasticDruckerPrager::donlyB(Real intnl, int fd, Real & dbbb) const
{
  if (_zero_phi_hardening && (fd == friction))
  {
    dbbb = 0;
...
void
RedbackPlasticDruckerPrager::dbothAB(Real intnl, Real & daaa, Real & dbbb) const
{
  if (_zero_cohesion_hardening && _zero_phi_hardening)
  {
    daaa = 0;
...
void
RedbackPlasticDruckerPrager::initializeAandB(Real intnl, Real & aaa, Real & bbb) const
{
  const Real C = _mc_cohesion.value(intnl);
  const Real cosphi = std::cos(_mc_phi.value(intnl));
  const Real sinphi = std::sin(_mc_phi.value(intnl));
...
void
RedbackPlasticDruckerPrager::initializeB(Real intnl, int fd, Real & bbb) const
{
  const Real s = (fd == friction) ? std::sin(_mc_phi.value(intnl)) : std::sin(_mc_psi.value(intnl));
  switch (_mc_interpolation_scheme)
  {
