...
addCrackFrontDefinitionParams(InputParameters & params)
{
  ...
  params.addRequiredParam<MooseEnum>("crack_direction_method",
                                     direction_method,
                                     "Method to determine direction of crack propagation.  Choices are: " +
                                       direction_method.getRawNames());
  params.addParam<MooseEnum>("crack_end_direction_method",
  ...
  params.addParam<RealVectorValue>("crack_direction_vector_end_1",
                                   "Direction of crack propagation for the node at end 1 of the crack");
  params.addParam<RealVectorValue>("crack_direction_vector_end_2",
                                   "Direction of crack propagation for the node at end 2 of the crack");
  params.addParam<std::vector<BoundaryName> >("crack_mouth_boundary",
  ...
  params.addRangeCheckedParam<unsigned int>("axis_2d",
                                            2,
                                            "axis_2d>=0 & axis_2d<=2",
                                            "Out of plane axis for models treated as two-dimensional (0=x, 1=y, 2=z)");
  params.addParam<unsigned int>("symmetry_plane", "Account for a symmetry plane passing through the plane of the "
                                                  "crack, normal to the specified axis (0=x, 1=y, 2=z)");
  ...
  if (end_direction_method_moose_enum.isValid())
  {
    ...
    if (_end_direction_method == END_CRACK_DIRECTION_VECTOR)
    {
      if (!isParamValid("crack_direction_vector_end_1"))
        mooseError(
          "crack_direction_vector_end_1 must be specified if crack_end_direction_method = CrackDirectionVector");
      if (!isParamValid("crack_direction_vector_end_2"))
        mooseError(
          "crack_direction_vector_end_2 must be specified if crack_end_direction_method = CrackDirectionVector");
...
CrackFrontDefinition::orderCrackFrontNodes(std::set<dof_id_type> & nodes)
{
  ...
  else // nodes.size() > 1
  {
    ...
    if (end_nodes.size() == 0) // Crack front is a loop.  Pick nodes to be end nodes.
    {
      ...
      if (_end_direction_method == END_CRACK_DIRECTION_VECTOR)
        mooseError(
          "In CrackFrontDefinition, end_direction_method cannot be CrackDirectionVector for a closed-loop crack");
    ...
    while (last_node != end_nodes[ 1 ])
    {
      ...
      for (unsigned int i = 0; i < curr_node_line_elems.size(); ++i)
      {
        ...
        for (unsigned int j = 0; j < curr_line_elem.size(); ++j)
        {
          ...
          if (_closed_loop &&
              (last_node == end_nodes[ 0 ] && line_elem_node == end_nodes[ 1 ])) // wrong direction around closed loop
            continue;
                                            ...
                                            std::vector<std::vector<dof_id_type> > & line_elems)
{
  ...
  // Find the two nodes connected to the node identified as the end node, and pick one of those to be the other end
  // node
...
CrackFrontDefinition::updateCrackFrontGeometry()
{
  ...
  else
  {
    ...
    if (hasAngleAlongFront())
    {
      ...
      {
        if (MooseUtils::absoluteFuzzyEqual(_angles_along_front[ 0 ], 0.0, _tol) && _angles_along_front[ 1 ] > 180.0)
    ...
    _console << "index   node id   x coord       y coord       z coord       x dir         y dir          z dir       "
                " angle        position     seg length"
             << std::endl;
    for (unsigned int i = 0; i < num_crack_front_points; ++i)
    {
      ...
      _console << std::left << std::setw(8) << i + 1 << std::setw(10) << point_id << std::setw(14)
               << (*getCrackFrontPoint(i))(0) << std::setw(14) << (*getCrackFrontPoint(i))(1) << std::setw(14)
               << (*getCrackFrontPoint(i))(2) << std::setw(14) << _crack_directions[ i ](0) << std::setw(14)
               << _crack_directions[ i ](1) << std::setw(14) << _crack_directions[ i ](2);
      if (hasAngleAlongFront())
...
CrackFrontDefinition::updateDataForCrackDirection()
{
  if (_crack_mouth_boundary_ids.size() > 0)
  {
    ...
    for (std::set<Node *>::iterator nit = crack_mouth_nodes.begin(); nit != crack_mouth_nodes.end(); ++nit)
    {
...
{
  if (!hasAngleAlongFront())
    mooseError("In CrackFrontDefinition, Requested angle along crack front, but not available.  Must specify "
                                                  ...
                                                  Real & theta) const
{
  ...
  if (_treat_as_2d)
  {
    // In 2D, the closest node is the crack tip node and the position of the crack tip node is (0,0,0) in the crack
...
CrackFrontDefinition::createQFunctionRings()
{
  ...
  if (_treat_as_2d) // 2D: the q-function defines an integral domain that is constant along the crack front
  {
    ...
    for (unsigned int ring = 2; ring <= _last_ring; ++ring)
    {
      ...
      for (std::set<dof_id_type>::iterator nit = old_ring_nodes_this_cfn.begin(); nit != old_ring_nodes_this_cfn.end();
           ++nit)
      {
  ...
  else // 3D: The q-function defines one integral domain around each crack front node
  {
    ...
    for (unsigned int icfn = 0; icfn < num_crack_front_points; ++icfn)
    {
      ...
      for (unsigned int ring = 2; ring <= _last_ring; ++ring)
      {
        ...
        // Find nodes connected to the nodes of the previous ring, but exclude nodes in rings of neighboring crack
        // front nodes
        std::set<dof_id_type> new_ring_nodes_this_cfn;
...
void
CrackFrontDefinition::addNodesToQFunctionRing(std::set<dof_id_type> & nodes_new_ring,
                                              ...
                                              std::vector<std::vector<const Elem *> > & nodes_to_elem_map)
{
  for (std::set<dof_id_type>::const_iterator nit = nodes_old_ring.begin(); nit != nodes_old_ring.end(); ++nit)
  {
    std::vector<const Node *> neighbors;
    MeshTools::find_nodal_neighbors(_mesh.getMesh(), _mesh.nodeRef(*nit), nodes_to_elem_map, neighbors);
    for (unsigned int inei = 0; inei < neighbors.size(); ++inei)
    {
      ...
      if (previt == nodes_all_rings.end() && thisit == nodes_neighbor1.end() && nextit == nodes_neighbor2.end())
