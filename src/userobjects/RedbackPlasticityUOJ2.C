...
void
RedbackPlasticityUOJ2::getFlowTensor(
  const RankTwoTensor & sig, Real q, Real p, Real yield_stress, RankTwoTensor & flow_tensor_dev) const
{
...
void
RedbackPlasticityUOJ2::getJac(const RankTwoTensor & sig,
                              ...
                              Real ref_pe_rate) const
{
  ...
  dfi_dseqv_dev = getDerivativeFlowIncrement(sig, yield_stress, dt, exponent, exponential, ref_pe_rate);
...
void
RedbackPlasticityUOJ2::get_py_qy(Real p, Real q, Real & p_y, Real & q_y, Real yield_stress) const
{
  p_y = p;
  q_y = yield_stress;
}
