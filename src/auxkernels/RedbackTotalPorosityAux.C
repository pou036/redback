...
template <> InputParameters validParams<RedbackTotalPorosityAux>() {
  ...
  params.addCoupledVar("mechanical_porosity", 0.0,
                       "Mechanical porosity (as AuxKernel)");
  return params;
}

RedbackTotalPorosityAux::RedbackTotalPorosityAux(
    const InputParameters &parameters)
    : AuxKernel(parameters),
      _is_mechanics_on(getParam<bool>("is_mechanics_on")),
      _delta_porosity_mech(
          _is_mechanics_on ? coupledValue("mechanical_porosity") : _zero),
