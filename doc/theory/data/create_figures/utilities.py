...
#     #plt.text(0.0, 1.01, '1e-3', fontsize=16, transform = plt.gca().transAxes)
  ...
  if do_show:
    ...
    if figure_filename is None:
      error_msg = 'You must provide a "figure_filename" since do_show=False'
      raise Exception, error_msg
    this_fig_dir, short_file_name = os.path.split(figure_filename)
...
def createFigsBenchmark(redback_dir,# redback directory
  ...
  for i_prop in range(nb_props):
    ...
    fig_shortfilename = '{0}_{1}_{2}.eps'.format(benchmark_dir_short_name,os.path.splitext(csv_shortfilename)[0],y_property_name)
    plotFigure(analytical_x_data=csv_gold_data[x_property_name],
               ...
               figure_filename=os.path.join(figures_dir,benchmark_dir_short_name,fig_shortfilename),
