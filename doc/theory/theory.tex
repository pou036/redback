...
\begin{table}
  \caption{List of parameters used in grain-size evolution loop}
\label{tab:grain_evo_params}
%\begin{tabular}{@{} l p{0.18\textwidth} l p{0.52\textwidth} @{}}
\begin{tabular}{| L{1cm} | C{3.75cm} | C{5.25cm} | C{5cm} |}
\hline\noalign{\smallskip}
...
\todo[inline]{What follows is a work flow of relevat from the 22/08/16, with user determined values for clacite only}
...
The loop below implements the theory from section ~\ref{section:theory_size_sensitivity}. At each timestep the grain-size is compared to the predicted steady state grain-size expected for the modelled deformation conditions. This loop is repated at each timestep until the observed grain-size is equal to the expected grain-size.\\
