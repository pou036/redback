...
\usetikzlibrary{shapes.geometric, arrows} % FOR FLOW CHART ARROWS
...
\end{enumerate}
\todo[inline]{ Note: Temperature in loop has dimentions but the code uses a dimensionless normalisation. TO DO! CODE LOOP TO NORMALISE TEMPERATURE}
...
\node (start) [startstop] {Start of timestep};
\node (in1) [io, below of=start, yshift=-0.5cm] {Extract Grain Size (\textcolor{red}{AuxVar}): $d$};
\node (in2) [io, left of=in1, xshift=-2cm] {Extract From Composite Flow Law (\textcolor{red}{UO}): $\dot{\varepsilon}_{tot}$, $K_{dis}$, $Q_{dis}$ $\&$ n};
\node (in3) [io, right of=in1, xshift=2cm] {Extract from Dislocation Flow Law (\textcolor{red}{AuxVar}): $\dot{\varepsilon}_{dis}$};
\node (pro1) [process, below of=start, yshift=-4cm, xshift=-2cm] {For extracted data use:\\ \textbf{$ d_{ss} = \kappa\sigma_d^{m'} exp^{\left(\frac{-Q_{'}}{RT}\right)}$}};
\node (dec1) [decision, below of=pro1, yshift=-1.5cm] {Does $d = d_{ss}$ };
\node (out1) [io, below of=dec1, yshift=-1.5cm] {Steady state grain-size has been acheived};
\node (pro2) [process, right of=dec1, xshift=5cm] {If, $d<d_{ss}$ use:\\ $\dot{d}_{growth}=K_g exp^{\left(\frac{-Q_g}{RT}\right)} p^{-1} d^{1-p}$};
\node (pro3) [process, right of=pro1, xshift=7cm] {If, $d>d_{ss}$ use:\\ $\dot{d}_{red}=\frac{\beta\lambda\sigma_{d}\dot{\varepsilon}d^2}{c\gamma}$};
...
\draw [arrow] (dec1) -- node[anchor=east] {\textbf{yes}} (out1);
\draw [arrow] (dec1) -- node[anchor=south] {\textbf{no}} (pro2);
\draw [arrow] (pro2) -- node[anchor=east] {\textbf{or}} (pro3);
\draw [arrow] (pro3) |- node[arrow, above] {\textbf{move one timestep forward}} (start);
