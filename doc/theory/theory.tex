...
\usetikzlibrary{shapes,shapes.geometric,arrows,fit,calc,positioning,automata,} % FOR FLOW CHART ARROWS
\usepackage{amsmath}
%\usepackage{ragged2e} % use for full justification eg.{\justifying\emph{Text I would....}\par}
...
%%% Normalised Dislocation creep flow law %%% 
\begin{equation}
...
%%%%%% Table of parameters for grain-size evoultion %%%%%% 
\begin{table}
\centering
...
$Q_g$ &  & $Q_g = 175 kJ mol^{-1}$ {\footnotesize \todo[inline]{(Covey-Crump, 1997)}} &  \\
...
\todo[inline]{What follows is a work flow of relevant from the 22/08/16 (James Gilgannon)}
...
%%% Rate of grain-size reduction %%%
\begin{equation}
\dot{d}_{red}=-\dot{W}_{dis}\lambda \frac{d^2}{c\gamma}
\end{equation}\\

%%% Rate of grain-size growth %%%
\begin{equation}
 \label{eq:d_growth}
...
Therefore; \:
%%% Competion of grain-size rates with red & growth terms %%%
\begin{equation}
 \label{eq:d_tot}
...
If , \: 
%%% Competion of grain-size rates at steady state %%%
\begin{equation}
 \label{eq:d_tot_0}
...
%%% Steady state grain-size flow law (?) %%%
\begin{equation}
...
\todo[inline, backgroundcolor=green]{Note: In \citet{Austin2007} Eq.10 contians error in $\kappa$ formulation, $\varepsilon_0$ is actually $K_{dis}$. See \citet{Herwegh2014} Eq.16 for the correct formulation of $\kappa$. This document contains the correct term definitions.}
...
\item a \textcolor{red}{UserObjectDislocationFlowLaw} must also be provided. This is required for the calculation of $\dot{\varepsilon}_{dis}$ (\textcolor{blue}{THIS FLOW LAW WILL BE ASKED FOR AGAIN IN THE MATERIAL}). The user will provide the relevant normalised parameters for the flow law. \todo[inline, backgroundcolor=green]{Note: The code should be reviewed to removed the need for the \textcolor{red}{UserObjectDislocationFlowLaw} to be entered twice.} 
...
\end{enumerate}
\todo[inline]{ Note: Temperature in loop has dimentions but the code uses a dimensionless normalisation. TO DO! CODE LOOP TO NORMALISE TEMPERATURE. 4.3e7*math.pow(1e6,1.1)*1e-2*math.pow(1e-6, -3.3)*math.exp(-2e5/(8.3*300))}
...
\begin{enumerate}
\item The current model intputs a $\bar{d}$ \& a variance (2$\sigma$) for each finite element. This is based on a gaussian log normal distubtion. However this fails to address a realsitic bias to small grain-sizes. Hence underestimating the contrubtion of diffusion creep. \todo[inline, backgroundcolor=green]{Is $\bar{d}$ actually the mean? Is it infact the mode? How do we better account for the real grain-size distrubtions observed in nature?}
...
\tikzstyle{input} = [trapezium, trapezium left angle=70, trapezium right angle=110, minimum width=1cm, minimum height=1cm, text centered, text width=1cm, draw=black, fill=blue!30]
\tikzstyle{timestep} = [rectangle, minimum width=3cm, minimum height=1cm,text centered, draw=black, fill=pink!30]
\tikzstyle{process} = [rectangle, minimum width=3cm, minimum height=2.5cm, text centered, text width=6cm, draw=black, fill=orange!30]
\tikzstyle{decision} = [diamond, minimum width=3cm, minimum height=1cm, text centered, draw=black, fill=green!30]
\tikzstyle{output} = [rectangle, minimum width=3cm, minimum height=1cm,text centered, draw=black, fill=red!30]
\tikzstyle{arrow} = [thick,->,>=stealth]


\begin{figure}
\centering
\caption{Grain-size evolution loop} 
\label{fig:gs_evolution_loop}
\begin{tikzpicture}[node distance=2cm]

\node (start1) [startstop] {Grain-size (AuxVar)};
\node (start2) [startstop, left of=start1, xshift=-50] {Flow Law (UO)};
\node (start3) [startstop, right of=start1, xshift=50] {Temperature (Var)};
\node (start4) [startstop, right of=start3, xshift=80] {Material};

\node (in1) [input, below of=start1, yshift=-1cm] {$d$};
\node (in2) [input, below of=start2, yshift=-1cm] {$Q_{dis}$, $K_{dis}$ \&  $n$};
\node (in3) [input, below of=start3, yshift=-1cm] {$T$};
\node (in4) [input, right of=in3, xshift=3cm] {$\dot{\varepsilon}_{tot}$ \& $\dot{\varepsilon}_{dis}$};

\node (pro1) [process, below of=in2, yshift=-2.5cm, xshift=2cm] {For extracted data use:\\ \textbf{$ d_{ss} = \kappa\sigma_d^{m'} exp^{\left(\frac{-Q_{'}}{RT}\right)}$}};
\node (dec1) [decision, below of=pro1, yshift=-2.5cm] {Does $d = d_{ss}$ };
\node (pro2) [process, right of=dec1, yshift=-1cm, xshift=5cm] {If, $d<d_{ss}$ use:\\ $\dot{d}_{growth}=K_g exp^{\left(\frac{-Q_g}{RT}\right)} p^{-1} d^{1-p}$};
\node (pro3) [process, right of=pro1, yshift=-2cm, xshift=7cm] {If, $d>d_{ss}$ use:\\ $\dot{d}_{red}=\frac{\beta\lambda\sigma_{d}\dot{\varepsilon}d^2}{c\gamma}$};

\node (out1) [output, below of=dec1, yshift=-2cm] {Steady state grain-size!};

\node(container1) [container, fit=(start1) (start2) (start3) (start4)] {};
\node at (container1)[left of=container1, xshift=-180, rotate=90] {User defined in .i};

\node(container2) [container, fit=(pro1) (pro2) (pro3)] {};
\node at (container2)[left of=container2, xshift=-180, rotate=90] {Grain-size Loop (AuxKern): RedbackGrainSizeAux};

\node(container3) [container, fit=(in1) (in2) (in3)] {};
\node at (container3)[left of=container3, xshift=-100, rotate=90] {User Inputs};

\node(container4) [container, fit=(in4)] {};
\node at (container4)[above of=in4, yshift=-15, rotate=0, text centered, text width=4cm] {Hidden Inputs from Material calculations};

\node (time1) [timestep, above of=container2, yshift=90] {TIME STEP};
...
\draw [arrow] (container3.south) |- (time1.west);
\draw [arrow] (container4.south) |- (time1.east);

\draw [arrow] (time1.south)-- ++(-.5,0) -- ++(0,-1)|-(pro1.east);
\draw [arrow] (pro3.west) -| node[pos=0.7,right] {\textbf{move to next TIME STEP}} (time1.south);

\draw [arrow] (pro1) -- (dec1);
\draw [arrow] (dec1) -- node[pos=0.3,left] {\textbf{yes}} (out1);
\draw [arrow] (dec1.east) -- node[anchor=south] {\textbf{no}} (pro2.west);
\draw [arrow] (pro2) -- node[anchor=east] {\textbf{or}} (pro3);


\end{tikzpicture}
\end{figure}

\chapter{Evolution of a second phase}
\label{chapter:evo_second_phase}

\todo[inline,backgroundcolor=green]{Read MOOSE for phase feild modelling}
