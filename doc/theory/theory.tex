...
\todo[inline]{What follows is a work flow of relevant from the 22/08/16, with user determined values for clacite only}
...
\todo[inline]{Note: In \citet{Austin2007} Eq.10 contians error in $\kappa$ formulation, $\varepsilon_0$ is actually $K_{dis}$. See \citet{Herwegh2014} Eq.16 for the correct formulation of $\kappa$. This document contains the correct term definitions.}
...
\todo[inline,backgroundcolor=green]{ Note: Temperature in loop has dimentions but the code uses a dimensionless normalisation. TO DO! CODE LOOP TO NORMALISE TEMPERATURE}
...
\node (start) [startstop] {Starting timestep};
\node (in1) [io, below of=start] {Extract:\\ \textbf {$\dot{\varepsilon},\:\sigma_d,\:T \: \& \: d$}};
\node (pro1) [process, below of=in1, yshift=-0.5cm] {For extracted data use:\\ \textbf{$ d_{ss} = \kappa\sigma_d^{m'} exp^{\left(\frac{-Q_{'}}{RT}\right)}$}};
\node (dec1) [decision, below of=pro1, yshift=-1.5cm] {Does $d = d_{ss}$ };
\node (out1) [io, below of=dec1, yshift=-0.5cm] {Steady state grain-size has been acheived};
\node (pro2) [process, right of=dec1, xshift=7cm] {If, $d<d_{ss}$ use:\\ $\dot{d}_{growth}=K_g exp^{\left(\frac{-Q_g}{RT}\right)} p^{-1} d^{1-p}$};
...
\draw [arrow] (pro3) |- node[arrow, above] {\textbf{move one timestep forward}} (in1);
\draw [arrow] (out1) -- (stop);

\end{tikzpicture}
\end{figure}

\section{Evolution of a second phase}
\label{section:evo_second_phase}
