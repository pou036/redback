...
out2[] = Extrude{ {0,0,1}, {0,0,0}, Pi/2}{ Surface{out1[0]};  Layers{nb_layers_quarter_circle}; Recombine;};
//out3[] = Extrude{ {0,0,1}, {0,0,0}, Pi/2-angle_crack/2}{ Surface{out2[0]};
//Layers{nb_layers_quarter_circle}; Recombine;};
...
Physical Surface (0) = {out1[4],out2[4]}; // top
Physical Surface (1) = {out1[2],out2[2]}; // bottom
Physical Surface (2) = {out1[3],out2[3]}; // outer boundary
Physical Surface (3) = {out1[5],out2[5]}; // inner boundary


Physical Surface (8) = {11}; // interface theta=0
Physical Surface (9) = {out2[0]}; // the other interface

Physical Volume(0) = {out1[1],out2[1]};
