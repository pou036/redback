[Mesh]
  type = FileMesh
  file = ../meshes/Cylinder_quarter_fine.msh
...
[Functions]
  active = 'sin_perturb timestep_function inner_pressure_fct outer_pressure_fct'
  [./temp_ic]
    type = ParsedFunction
  ...
  [./inner_pressure_fct]
    type = ParsedFunction
    value = 1e-2 # 1e-3+4e-2*t
  [../]
  [./timestep_function]
    # if(t>0.0225, 5e-5, 5e-4)
    # 1e-3
    type = ParsedFunction
    value = 'if (t<0.15,1e-3,  dt*max(0.2, 1-0.1*(n-30)))' # if(t>0.0169, 1e-7, if(t>0.0168, 2e-6,1e-4))
  ...
  [./sin_perturb]
    type = ParsedFunction
    value = 0.05*(1+sin(18*asin(y/sqrt(x*x+y*y))))
  [../]
[]
...
[Postprocessors]
  active = 'disp_hole nnli max_r_theta new_timestep nli old_timestep'
  [./max_temp]
    type = NodalMaxValue
    variable = temperature
  [../]
...
[Outputs]
  ...
  file_base = Quarter_damage_sin_perturb_fine1
  interval = 10
[]
