[Mesh]
  type = FileMesh
  file = Cylinder_hollow_perturb_fine_2D_dense_no_perturb.msh
...
[Functions]
  active = 'inner_displacement timestep_function inner_pressure_fct'
  [./temp_ic]
    type = ParsedFunction
  ...
  [./timestep_function]
    ...
    type = ParsedFunction
    value = 'if(t>0.04947, 1e-8, if(t>0.0493, 1e-7, if(t>0.04, 2e-4, max(1e-7,min(1e-2, dt*max(0.2, 1-5*(T-T_old-0.2)))))))'
...
[Outputs]
  ...
  file_base = borehole_dmg_2D_test3_noperturb
[]
