...
public:
  ...
  void
  fillFromInputVector(const std::vector<Real> & input)
  {
    if (input.size() != 6)
    {
  ...
  Real
  rowDot(const unsigned int r, const libMesh::TypeVector<Real> & v) const
  {
  ...
  Real
  doubleContraction(const SymmTensor & rhs) const
  {
    return _xx * rhs._xx + _yy * rhs._yy + _zz * rhs._zz + 2 * (_xy * rhs._xy + _yz * rhs._yz + _zx * rhs._zx);
  }
  ...
  {
    return _xx == rhs._xx && _yy == rhs._yy && _zz == rhs._zz && _xy == rhs._xy && _yz == rhs._yz && _zx == rhs._zx;
  }
  bool operator!=(const SymmTensor & rhs) const
  {
    return !operator==(rhs);
  ...
  static SymmTensor
  genRandomSymmTensor(Real scalefactor)
  {
